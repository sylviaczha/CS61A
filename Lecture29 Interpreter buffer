...
class Buffer(object):
    ...
    and then supplies the items from them one at a time through its pop() method,
    calling the source for more sequences of items only when needed.

    In addition, Buffer provides a current() method to look at the next item to 
    be supplied, without sequencing past it.
...
class InputReader(object):
    """An InputReader is an interable that prompts the user for input."""
