...
def next_candidate_token(line, k):
    """A tuple (tok, k'), where tok is the next substring of line at or after
    that could be a token (assuming it passes a validity check), and k' is 
    the position in line following that token.
...
def tokenize_line(line):
    ...
    while text is not None:
        ...
        elif text[0] in _SYMBOL_STARTS and valid_sybol(text):
            result.append(text)
        else:
